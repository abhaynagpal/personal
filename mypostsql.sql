--INSERT INTO `mpc-dev-459470.DELEVERIES.C_TMP_EM_PIVOT` SELECT * FROM `mpc-dev-459470.DELEVERIES.MC_BASE_PROC_VIEW`;
...
WITH
  ...
  WHERE e.EDAP_RECORD_DATE BETWEEN @FromDate AND @ToDate
  ...
  WHERE
    ...
    FROM
      guids)
    AND  e. EDAP_RECORD_DATE BETWEEN @FromDate AND @ToDate
  ...
  FROM (
    ...
    FROM
      all_events
    WHERE
      EDAP_RECORD_DATE BETWEEN @FromDate AND @ToDate
      AND (EXISTS (
        SELECT
          1
        FROM
